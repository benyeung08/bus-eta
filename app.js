...
function kmbTimetableNext(route, bound){
  ...
  if(!byMask && bound!=='O' && bound!=='I'){
    ...
    if(a && b){ const x=_freqCalc(a), y=_freqCalc(b);
      return (x===null)?y:((y===null)?x:Math.min(x,y)); }
    byMask = a || b;
  }
  if(!byMask) return null;
  return _freqCalc(byMask);
}
function _freqCalc(byMask){
  ...
  for(const line of parts[0].split('\n')){
    ...
    for(const sid of stops){
      ...
      if(!arr.some(x=>x.route===route && x.bound===bound))
        arr.push({route, bound: bound||''});   // 保留方向：篩選與班次表都需要
    }
  }
  const stops = [];
...
async function fetchStopEta(){
  ...
  box.innerHTML = `<div class="card">
    ...
    ${nSched? `<div class="banner" style="margin:6px 0 2px">📋 實時到站${allSched?'無法取得':'部分缺資料'}，標示「班表推算」者為依<b>班次表</b>估算的頭站開出時間，<b>未計入行車時間</b>，僅供參考。</div>`:''}
...
async function ctbStopEta(stopId){
  ...
  let routes = (D.CTB.routesAtStop?.get(String(stopId)) || []).map(r=>({route:r.route, dir:r.bound}));
  if(!routes.length){
    for(const mk of EP.ctbStopRoute){
      try{
...
async function kmbStopEta(id){
  ...
  return list.map(x=>{
    ...
    if(r.min===null || r.min===undefined){
      ...
      const m = kmbTimetableNext(x.route, x.bound);
...
function renderDataStatus(){
  ...
  el.innerHTML = `車站／路線索引：${ok.map(n=>`<span class="ok">✓${n}</span>`).join(' ')}` +
    ...
    `<div class="tiny muted" style="margin-top:3px">小巴／港鐵巴士／城巴／嶼巴／輕鐵／重鐵／渡輪的車站與路線為內建資料（GitHub 開放資料）。渡輪無實時到站，顯示為時刻表推算之下一班。</div>`;
  if(!ok.length){
