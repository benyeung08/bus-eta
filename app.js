...
function kmbTimetableNext(route, bound){
  if(!KMB_FREQ.ready) return null;
  const byMask = KMB_FREQ.map.get(route+'|'+(bound||''));
  ...
  for(let pass=0; pass<2; pass++){                 // 第二段跨越午夜（如 2445）
    for(const p of per){
      ...
      if(e <= cur) continue;
      if(s > cur){
        if(s-cur <= KMB_FREQ_MAX) soonest = soonest===null ? s-cur : Math.min(soonest, s-cur);
        continue;
      }
      const dep = s + Math.ceil((cur - s) / p[2]) * p[2];
      if(dep < e && dep-cur <= KMB_FREQ_MAX) soonest = soonest===null ? dep-cur : Math.min(soonest, dep-cur);
    }
...
function kmbPickEta(data, it){
  ...
  if(dir){
    const f = arr.filter(e=>{ const d = pick(e,'dir'); return !d || String(d)===dir; });
    if(f.length) arr = f;                                  // 沒 dir 欄位時不篩
  }
...
async function kmbRouteEta(stopId, it){
  ...
  const e = cand.find(x=>pick(x,'eta','timestamp')) || cand[0];
  if(!e) return null;
  return {route:it.route, min: minsTo(pick(e,'eta','timestamp')),
          iso: pick(e,'eta','timestamp') || null,
          dest: pick(e,'dest_tc') || routeDest('KMB', it.route, it.bound) || '',
          rmk: pick(e,'rmk_tc') || ''};
}
async function kmbStopEta(id){
  // 1) 整站端點（1 次請求）— 大多數情況這裡就有答案
  let rows = [];
  try{
    const j = await getJSON(EP.kmbStopEta(id), {timeout:15000});
    rows = asArray(pick(j,'data')).map(d=>{
      const arr = asArray(pick(d,'eta'));
      // 取第一個有時間的班次，不要死用 [0]
      const e = arr.find(x=>pick(x,'eta','timestamp')) || arr[0] || {};
      return {route:pick(d,'route'),
              min: toMin(pick(e,'diff_min','diff')) ?? minsTo(pick(e,'eta','timestamp')),
              iso: pick(e,'eta','timestamp') || null,
  ...
  // 2) 沒有到站時間的路線，改用逐路線端點重查（官方用法）
  const idx = D.KMB.routesAtStop?.get(String(id)) || [];
  if(!idx.length) return rows;
  const need = idx.filter(it=>{
    const r = rows.find(x=>String(x.route)===String(it.route));
    return !r || r.min===null || r.min===undefined;
  });
  if(!need.length) return rows;
  const got = await pool(need.slice(0,20), 5, async it=>{
    try{ return await kmbRouteEta(id, it); }catch(_){ return null; }
  });
  const fixed = got.filter(Boolean);
  // 以逐路線結果覆寫（有時間的優先），並補上原本缺少的路線
  let out = rows.map(r=>{
    if(r.min!==null && r.min!==undefined) return r;
    return fixed.find(x=>String(x.route)===String(r.route)) || r;
  });
  for(const f of fixed) if(!out.some(x=>String(x.route)===String(f.route))) out.push(f);
  // 3) 仍沒有到站時間的（實時 API 真的沒資料），改用班次表推算
  out = out.map(r=>{
    if(r.min!==null && r.min!==undefined) return r;
    const it = idx.find(x=>String(x.route)===String(r.route));
    if(!it) return r;
    const m = kmbTimetableNext(it.route, it.bound);
    return m===null ? r : Object.assign({}, r, {min:m, sched:true, iso:null});
  });
  return out;
}
